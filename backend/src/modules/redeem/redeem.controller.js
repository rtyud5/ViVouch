...
export const checkVoucherCode = asyncHandler(async (req, res) => {
  ...
  const result = await checkCode(req.user.userId, req.partnerAccess, code, branchId);
  res.status(200).json({
    success: true,
...
export const confirmVoucherCode = asyncHandler(async (req, res) => {
  const { code, branchId } = redeemVoucherCodeSchema.parse(req.body);
  const result = await confirmCode(req.user.userId, req.partnerAccess, code, branchId);
  res.status(200).json({ success: true, message: 'Xác thực voucher thành công', data: result });
});
