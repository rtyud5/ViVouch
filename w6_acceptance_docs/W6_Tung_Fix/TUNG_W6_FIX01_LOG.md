...
> [!NOTE]
> **Về "5 false positives" trong báo cáo đầu vs "30+ dòng" trong grep đầy đủ:** Báo cáo đầu dùng regex chặt hơn (`local-file-example[A-Za-z]:|[A-Za-z]:[/\\][A-Za-z0-9]`) và chỉ paste kết quả trong phạm vi W6 docs — nên chỉ thấy 5 dòng. Báo cáo bổ sung theo yêu cầu review dùng **đúng regex gốc của task spec** (`local-file-example|[A-Za-z]:[/\\]`) — regex này rộng hơn, khớp thêm cả `http://`, `https://`, `postgresql://` (vì `[A-Za-z]:` match bất kỳ URL scheme nào), và quét **toàn bộ 170 file**. 30+ dòng là kết quả của hai yếu tố cộng lại: regex rộng hơn + phạm vi rộng hơn. Kết luận không thay đổi: **không còn dòng nào dạng `[text]`local-file-example...`` hay `[text]`windows-path-example``** trong toàn bộ 170 file.
...
> **Lưu ý nhất quán với báo cáo đầu:** Báo cáo đầu ghi "5 false positives" vì dùng regex chặt hơn (`local-file-example[A-Za-z]:|[A-Za-z]:[/\\][A-Za-z0-9]`) và chỉ paste kết quả W6 docs (5 dòng). Grep này dùng **regex gốc của task spec** (`local-file-example|[A-Za-z]:[/\\]`) quét toàn bộ 170 file — ra 30+ dòng vì regex rộng hơn còn khớp `http://`, `https://`, `postgresql://`. Trong cả hai trường hợp, kết luận như nhau: **zero actual Markdown hyperlink trỏ absolute local path**.

Lệnh chạy:
```bash
git grep -nEi 'local-file-example|[A-Za-z]:[/\\]' -- '*.md'
```
...
W6_HUY_REPORT.md:24:  - Chặn các đường dẫn local tuyệt đối (ví dụ `local-file-example`).
...
w6_acceptance_docs/W6D5/W6_INHERITED_REMEDIATION_FOR_W7.md:82:- link dạng `local-file-example/C:/...` hoặc `local-file-example/D:/...`;
w6_acceptance_docs/W6D5/W6_INHERITED_REMEDIATION_FOR_W7.md:190:- W6 có link local `local-file-example/...`;
w6_acceptance_docs/W6D5/W6_INHERITED_REMEDIATION_FOR_W7.md:211:   - `local-file-example`, đường dẫn Windows tuyệt đối hoặc đường dẫn máy cá nhân;
w6_acceptance_docs/W6D5/W6_INHERITED_REMEDIATION_FOR_W7.md:227:| EV-03 | Link `local-file-example/D:/...` | Exit code khác 0 |
w6_acceptance_docs/W6D5/W6_INHERITED_REMEDIATION_FOR_W7.md:261:- link local `local-file-example` bị chặn;
w6_acceptance_docs/W6D5/W6_INHERITED_REMEDIATION_FOR_W7.md:1022:- không dùng `local-file-example`;
w6_acceptance_docs/W6D5/W6_INHERITED_REMEDIATION_FOR_W7.md:1135:local-file-example
w6_acceptance_docs/W6D5/W6_INHERITED_REMEDIATION_FOR_W7.md:1145:git grep -nEi 'local-file-example|[A-Za-z]:[/\\\\]' -- '*.md'
w6_acceptance_docs/W6D5/W6_INHERITED_REMEDIATION_FOR_W7.md:1167:\LOCAL_EVIDENCE_PATH
w6_acceptance_docs/W6D5/W6_RISK_WAIVER_REGISTER.md:27:| W6-DOC-001 | P2 | T5 documents chứa `local-file-example/d:/...`, không dùng được ngoài máy tác giả | W6T5 docs | Tùng | Đổi toàn bộ sang repository-relative links; verifier pass | Không cần waiver nếu sửa cùng inherited PR |
...
|-----------|---------------|-----------------|
| `README.md:31–33` | `https://nodejs.org`, Docker, Git | URL HTTPS công khai — không phải `local-file-example` hay Windows path tuyệt đối |
| `README.md:42` | `https://github.com/...` | URL HTTPS công khai |
| `README.md:102,109,116–119,130–131,158` | `http://localhost:*`, `postgresql://...` | URL dev server / connection string — không phải Markdown link `[text]`local-file-example...`` |
| `W6_HUY_REPORT.md:24` | `` `local-file-example` `` | Inline backtick — text mô tả, không phải ```[text]`example-url```` |
...
| `w5_acceptance_docs/**` | `http://localhost:5173/...` | URL dev server trong script demo W5 — không phải `local-file-example` |
| `w6_acceptance_docs/W6D1/**:67–68` | `curl -fsS http://localhost:3000/...` | Lệnh shell trong code block — không phải Markdown link |
| `W6_INHERITED_REMEDIATION_FOR_W7.md:82` | `` `local-file-example/C:/...` hoặc `local-file-example/D:/...` `` | Inline backtick trong bullet list — ví dụ minh họa Evidence không hợp lệ |
| `W6_INHERITED_REMEDIATION_FOR_W7.md:190` | `` `local-file-example/...` `` | Inline backtick — criteria |
| `W6_INHERITED_REMEDIATION_FOR_W7.md:211` | `` `local-file-example` `` | Inline backtick — criteria |
| `W6_INHERITED_REMEDIATION_FOR_W7.md:227` | `` `local-file-example/D:/...` `` | Trong ô bảng, backtick — test input ví dụ EV-03 |
| `W6_INHERITED_REMEDIATION_FOR_W7.md:261` | `` `local-file-example` `` | Inline backtick — criteria |
| `W6_INHERITED_REMEDIATION_FOR_W7.md:1022` | `` `local-file-example` `` | Inline backtick — quy tắc |
| `W6_INHERITED_REMEDIATION_FOR_W7.md:1135` | `local-file-example/D:/...` | Trong **```text** code fence (dòng 1134–1136) — ví dụ minh họa của chính FIX-01 |
| `W6_INHERITED_REMEDIATION_FOR_W7.md:1145` | `` git grep -nEi 'local-file-example|...' `` | Trong **```bash** code fence (dòng 1144–1146) — lệnh shell trong hướng dẫn |
| `W6_INHERITED_REMEDIATION_FOR_W7.md:1167` | `\LOCAL_EVIDENCE_PATH` | Trong **```markdown** code fence (dòng 1166–1168) — ví dụ "không được dùng", có ký tự thoát `\` |
| `W6_RISK_WAIVER_REGISTER.md:27` | `` `local-file-example/d:/...` `` | Backtick trong ô bảng — mô tả rủi ro, không phải hyperlink |
...
> [!IMPORTANT]
> **Không còn dòng nào trong toàn bộ 170 file có dạng `[link text]`local-file-example...`` hoặc `[link text]`windows-path-example``** — syntax Markdown hyperlink thực sự trỏ tới local absolute path.
...
**Xác nhận scope:** `git grep -- '*.md'` quét **toàn bộ 170 file** — bao gồm đủ cả 16 file W6T1–T5. Trong 16 file đó, chỉ `W6T1_03_negative_scenarios.md:34,53` xuất hiện trong grep output do chứa `http://localhost:3000/...` trong lệnh curl (không phải `local-file-example` hay Windows absolute path). **Không có file T1–T5 nào chứa absolute local Markdown link.** 31 link cần sửa chỉ nằm trong `walkthrough.md` — kết luận này là kết quả của quét toàn bộ, không phải do phạm vi bị thu hẹp.
...
**Pattern nhất quán trên toàn bộ 31 thay đổi:** xóa prefix `../../` khỏi Markdown link URL, giữ nguyên phần path `backend/...`. Không có thay đổi nào với text hiển thị, ký tự Unicode, hay nội dung code block.
