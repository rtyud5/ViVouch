...
| **`verifyToken`** (Data Validation) | Kiểm tra `user` tồn tại trong database và trạng thái. Mã lỗi trả về `403` nếu bị khóa ngang. | `403` | `ACCOUNT_LOCKED` | Khi admin khóa user/partner, layer `verifyToken` chặn các API dùng Access Token còn hạn (ném `403 ACCOUNT_LOCKED`). Đối với luồng refreshing token, check-status kiểm soát tài khoản khóa phải được tích hợp riêng tại endpoint đó do không đi qua `verifyToken` tiêu chuẩn. |
...
| **`getApprovedPartner...()`** (Business Layer Guard) | Các thao tác tạo/sửa đổi voucher và đối soát mã (redeem) yêu cầu Partner phải `APPROVED`. | `403` | `PARTNER_NOT_ACTIVE` / `FORBIDDEN` | Trả về `PARTNER_NOT_ACTIVE` đối với tạo/sửa/duyệt voucher (qua `getApprovedPartnerByUserId`); trả về `FORBIDDEN` đối với redeem mã. Thao tác cá nhân (Profile, Branches) không yêu cầu `APPROVED`. |
...
|:---|:---|:---:|
| **Customer xem Order / Giỏ hàng** | `WHERE userId = req.user.userId` trực tiếp trong Prisma Query. | `404 / Trống` |
